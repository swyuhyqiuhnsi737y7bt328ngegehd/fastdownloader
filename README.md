...
- **手动指定**：添加任务时把 64 位哈希粘进"校验 SHA256"框（剪贴板里正好是哈希时会自动填入）；
  命令行用 `--sha256 <哈希>`。
- **自动探测**：默认会尝试下载 `<文件URL>.sha256` / `.sha256sum`（很多开源项目都附带），
  取到哈希就自动校验；可在 设置 → 常规 里关闭（关掉可省一次请求）。
- **校验时机**：在文件**改名前**进行——不通过就报错并保留 `.part`，**不会产出损坏的最终文件**，
  清理或换个源后重下即可。
- 命令行加 `--hash` 可以先不比对、只要结果哈希（便于自己核对或写进脚本）。
...
```
fastdownloader/
├── main.py                 # 程序入口（无参数启动 GUI，带链接/参数走 CLI）
├── cli.py                  # 命令行下载器（不依赖 PyQt5）
...
命令行模式**不加载图形界面**（不导入 PyQt5），启动快，适合脚本、计划任务、服务器。

```bash
# 单个文件
python cli.py https://example.com/big.zip -o D:\\Downloads

# 多个链接并发下载到目录
python cli.py URL1 URL2 -o D:\\Downloads -j 4

# 从文件读取链接（每行一个，# 开头为注释）
python cli.py -i urls.txt -o D:\\Downloads

# 校验 SHA256（留空时会自动尝试服务器上的 <文件>.sha256）
python cli.py https://example.com/f.iso --sha256 <64位哈希>

# JSON 输出 + 输出文件哈希，方便脚本处理
python cli.py URL -o D:\\dl --json --hash
```

打包版同样可用——程序没有控制台窗口，会**自动挂接到调用它的那个控制台**：
...
| 选项 | 说明 |
|------|------|
| `-o, --output` | 保存路径。单个链接可以是文件或目录；多个链接给目录 |
| `-j, --jobs` | 同时下载的任务数（默认 3） |
| `-t, --threads` | 单任务分片线程数（默认取设置） |
| `--sha256 HASH` | 期望哈希，不匹配则失败且不产出文件 |
| `--hash` | 下载完成后输出实际 SHA256（不比对） |
| `--proxy` / `-H` | 代理 / 附加请求头（可重复） |
| `-s, --speed-limit` | 全局限速 KB/s |
| `--conflict` | 目标已存在时：`rename`（默认命令行是 `overwrite`）/ `overwrite` / `skip` |
| `--json` | 输出结构化结果 |
| `--no-progress` | 不显示进度 |
